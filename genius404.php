<?php
/*
Plugin Name: genius 404
Plugin URI: http://atastypixel.com/blog/wordpress/plugins/genius-404/
Description: It's an alternative for smart 404 , which can help you to  rescue your viewers from site errors!  When content cannot be found, genius 404 will use the current URL to attempt to find matching content, and redirect to it automatically. genius 404 also supplies template tags which provide a list of suggestions, for use on a 404.php template page if matching content can't be immediately discovered.
Version: 1.0
 ...
 */
function genius404_search($search, $type) {
    $search_words = trim(preg_replace( "@[_-]@", " ", $search));
 ...
 *
 */
function genius404_redirect_canonical_filter($redirect, $request) {
 ...
 * @since 0.1
 */
function genius404_setup_admin() {
	add_options_page( 'genius 404', 'genius 404', 5, __FILE__, 'genius404_options_page' );
 ...
 * @since 0.1
 */
function genius404_options_page() {
	?>
	<div class="wrap">
	<h2>genius 404</h2>
 ...
 */
function genius404_has_suggestions() {
	return ( isset ( $GLOBALS["__genius404"]["suggestions"] ) && is_array( $GLOBALS["__genius404"]["suggestions"] ) && count( $GLOBALS["__genius404"]["suggestions"] ) > 0 ); 
}
 ...
 */
function genius404_get_suggestions() {
	return $GLOBALS["__genius404"]["suggestions"];
}
 ...
 */
function genius404_suggestions($format = 'flat') {
	if ( !isset ( $GLOBALS["__genius404"]["suggestions"] ) || !is_array( $GLOBALS["__genius404"]["suggestions"] ) || count( $GLOBALS["__genius404"]["suggestions"] ) == 0 ) 
		return false;
	
	echo '<div id="genius404_suggestions">';
	if ( $format == 'list' )
		echo '<ul>';
		
	foreach ( (array) $GLOBALS["__genius404"]["suggestions"] as $post ) {
		if ( $format == "list" )
			echo '<li>';
 ...
 */
function genius404_loop() {
	if ( !isset ( $GLOBALS["__genius404"]["suggestions"] ) || !is_array( $GLOBALS["__genius404"]["suggestions"] ) || count( $GLOBALS["__genius404"]["suggestions"] ) == 0 ) {
		return false;
	}
	
	$postids = array_map(create_function('$a', 'return $a->ID;'), $GLOBALS["__genius404"]["suggestions"]);
 ...
 */
function genius404_get_search_terms() {
    return $GLOBALS["__genius404"]["search_words"];
}

// Set up plugin

add_action( 'template_redirect', 'genius404_redirect' );
add_filter( 'redirect_canonical', 'genius404_redirect_canonical_filter', 10, 2 );
add_action( 'admin_menu', 'genius404_setup_admin' );
